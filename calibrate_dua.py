# --- START OF FILE calibrate_from_folder.py (ENHANCED DISPLAY & STANDARD MODEL) ---
...
def calibrate_camera_for_intrinsic_parameters(image_paths, screen_res):
    ...
    (MODIFIED to use STANDARD 5-parameter distortion model)
    """
    if not image_paths:
    ...
    # --- MODIFICATION: Reverted to the standard distortion model ---
    # By removing the `flags` argument, OpenCV will use its default 5-parameter
    # model (k1, k2, p1, p2, k3). This is more robust against overfitting if
    # the calibration image set is not perfectly diverse.
    print("\nCalibrating with standard 5-parameter distortion model...")
    ...
    # --- Per-view error analysis for diagnostics (still very useful!) ---
