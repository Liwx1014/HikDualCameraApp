...
class CameraOperation():
    ...
    def __init__(self,obj_cam,st_device_list,n_connect_num=0,b_open_device=False,b_start_grabbing = False,h_thread_handle=None,\
        ...
        self.capture_datetime = None # 用于存储采集时刻的datetime对象
    ...
    def work_thread(self, n_index, win_handle, exit_flag):
        ...
        while not exit_flag.is_set():
            ...
            if 0 == ret:
                # 拷贝图像和图像信息
                # 获取缓存锁
                current_capture_time = datetime.now() 
          
                self.buf_lock.acquire()
                self.capture_datetime = current_capture_time # 将该时间保存到成员变量
    ...
    # 存BMP图像
    def save_bmp(self, base_save_folder):
        if 0 == self.buf_save_image:
            return MV_E_NODATA # Return an error code for no data

        # 获取缓存锁
        self.buf_lock.acquire()
        
        # Create a specific folder for this camera
        cam_folder = os.path.join(base_save_folder, f"camera_{self.n_connect_num}")                     
        os.makedirs(cam_folder, exist_ok=True)
        # --- 使用准确的采集时间 ---
        timestamp_str = self.capture_datetime.strftime("%Y%m%d_%H%M%S_%f")[:-3]
    
        file_name = f"{timestamp_str}_cam{self.n_connect_num}_frame{self.st_frame_info.nFrameNum}.bmp"
        ...
        stSaveParam.iMethodValue = 1 # Use high quality for BMP
